package org.flixel
{
	...
	public class FlxButton extends FlxGroup
	{
		 ...
		 */
		public function FlxButton(X:int,Y:int,Callback:Function, Height:uint=20, Width:uint=100, offColor:uint=0xff3e6a7a, onColor:uint=0xff1a5569)
		{
			super();
			x = X;
			y = Y;
			width = Width;
			height = Height;
			_off = new FlxSprite().createGraphic(width, height, offColor);
			_off.solid = false;
			add(_off, true);
			_on  = new FlxSprite().createGraphic(width,height,onColor);
			_on.solid = false;
			add(_on,true);
			_offT = null;
			_onT = null;
